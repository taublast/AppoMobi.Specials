...
public class ResourceEnumConverter : EnumConverter
{
	...
	public ResourceEnumConverter(Type type, ResourceManager resourceManager)
		: base(type)
	{
		var res = type.GetAttribute<FromResources>()?.Type;
		if (res != null)
			_resourceManager =
				new ResourceManager(res.FullName, res.Assembly);
		//                    strBuffer = HtmlProcessor.ApplyIndexTemplate(myTemplate, item, propVal, clientLang);
		else
			_resourceManager = resourceManager;

		var flagAttributes = type.GetCustomAttributes(typeof(FlagsAttribute), true);
		_isFlagEnum = flagAttributes.Length > 0;
		if (_isFlagEnum) _flagValues = Enum.GetValues(type);
	}
	...
	private LookupTable GetLookupTable(CultureInfo culture)
	{
		...
		if (!_lookupTables.TryGetValue(culture, out result))
		{
			...
			foreach (var value in GetStandardValues())
			{
				...
				if (text != null) result.Add(text, value);
			}
	...
	private string GetFlagValueText(CultureInfo culture, object value)
	{
		...
		//
		if (Enum.IsDefined(value.GetType(), value)) return GetValueText(culture, value);
	...
	private object GetFlagValue(CultureInfo culture, string text)
	{
		...
		foreach (var textValue in textValues)
		{
			...
			if (!lookupTable.TryGetValue(trimmedTextValue, out value)) return null;
	...
	public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
	{
		if (value is string)
		{
			var result = _isFlagEnum ? GetFlagValue(culture, (string)value) : GetValue(culture, (string)value);
			if (result == null) result = base.ConvertFrom(context, culture, value);
			return result;
		}
	...
	/// <summary>
	///     Return a list of the enum values and their associated display text for the given enum type in the current UI
	///     Culture
	/// </summary>
	/// <param name="enumType">The enum type to get the values for</param>
	/// <returns>
